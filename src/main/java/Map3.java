...
public class Map3 extends Mapper<Object, Text, Text, Text> {
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (values.length > 4) {
            final String dir = format(values[5]);
            // Verificar si la dirección es válida y tiene múltiples partes
            if (dir != null) {
                final String[] dirValues = dir.toString().split(SEPARATOR2);
                if (dirValues.length > 1) {
                    // Extraer la ciudad de la dirección
                    final String ciudad = format(dirValues[1]);
                    ...
                    // Emitir el par clave-valor (ciudad, cantidadProductos)
                    context.write(new Text(ciudad), new Text(values[1]+":"+cantidadProductos.toString()));
                }
            }
        }
    }
